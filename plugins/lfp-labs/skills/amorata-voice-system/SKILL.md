...
description: >
  ...
  "EXPERT_VOICE", "word ranges", "amorata voice", "dual voice", "voz tcnica", "combinator",
  "section weight", "capa de seccin", "astro commentary", "carta text quality",
  ...
  NOT masterkey (a general creative process): this loads Amorata's dual-voice architecture for the Carta natal project only.
metadata:
  intent: reason
---
...
Amorata genera **dos textos complementarios** por seccin: una voz literaria (INTERPRETER) y
una voz tcnica (EXPERT). Nunca se reemplazan  siempre coexisten. La literaria abre; la
tcnica valida.

**Origen del split:** Giulina (usuaria real, Phase 0, 2026-06-18): "el lenguaje es demasiado
abstracto nada se entiende se siente fantasa nada serio ni tcnico."

---

## 1. WORD_RANGES  Extensin por capa

La extensin no es fija por voz  es funcin de la **capa de la seccin**. Ambas voces
...
WORD_RANGES = {
    ...
    "SOCIAL":   {"literary": (30, 50),   "expert": (20, 35)},   # Jpiter, Saturno
    "OUTER":    {"literary": (15, 25),   "expert": (10, 18)},   # Urano, Neptuno, Plutn
    "MODIFIER": {"literary": (15, 20),   "expert": (10, 15)},   # Quirn, Llith, Nodos
    "ASPECT":   {"literary": (25, 40),   "expert": (15, 25)},   # Aspectos mayores
    "TRANSIT":  {"literary": (12, 20),   "expert": (8, 12)},    # Trnsitos activos
}
```

**Layer mapping**  aadir `layer` como 6 campo en `SECCIONES_DEF`:
...
(27, "Trnsitos...", None,      None,      None,      "TRANSIT"),
(28, "Sntesis...",  None,      None,      None,      "TRUNK"),
```
...
- Registro: literario, imagen viva, emocional, espaol LATAM
- Referencia: Rowling  Martha Stewart
- Funcin: abre cada seccin, habla al corazn
- Nunca: "manifestar", "vibra", "energa csmica", "despertar espiritual"
...
Resumen de identidad:
- Registro: tcnico, posicional, diagnstico
- Referencia: Rob Hand  Susan Miller
- Funcin: cierra cada seccin, habla a la cabeza
- Reglas: sin metfora, sin hedge ("puede ser que"), sin preguntas retricas
- Formato: signo + casa + dignidad + consecuencia prctica
- Palabras tcnicas permitidas y esperadas: dignidad, domicilio, exaltacin, cada, detrimento, angular, sucedente, cadente, orbe, aplicante, separante, rx

---

## 4. Implementacin en combinator.py
...
**Firestore schema**  cada seccin almacena:
```json
{
  "numero": 1,
...
- No define por negacin ("X no es Y  es Z")
- Cierra con segunda persona directa y especfica para el placement

### EXPERT (voz B)
- Siempre menciona signo + casa + dignidad si aplica
- Consecuencia prctica obligatoria ("Resultado:", "En prctica:", o implcito)
- Menciona aspectos relevantes si estn en contexto
- Rx siempre sealado cuando aplica
- No ms palabras de las que el rango permite  densidad sobre extensin

---

## 6. Banderas de violacin (_check_voice_lock)

El sistema actualmente valida voz A. Extender para voz B:

**Voz B violations:**
- Contiene "puede ser que", "quizs", "tal vez", "a veces puede"
- Usa "sientes", "llevas", "tu" (segunda persona  la experta es descriptiva, no directa)
- No menciona signo o casa para secciones 1-13
- Supera el mximo de palabras del layer
