...
description: >
  ...
  "subttulo", "tagline", "CTA", "microcopy", "UX writing", "voz de marca", "brand voice", "el copy
  suena genrico", "the creative side", "make the copy better", "rewrite this hero", "ms vendedor" 
...
One subtlety: a new audience can tempt you to shift register  e.g. moving a brand's established *t*
...
top of the deck ("la home usa t; esta audiencia B2B podra pedir usted  confirmas?") so the user
...
on a page ("140K usuarios", "cierre en 3 das", "+40% conversin") is a claim. If you can't point to a
...
Per is not Colombia; S/ is not COP is not USD; "carro" vs "auto" vs "coche" signals whether you know
...
con cierre auditable en das" says everything. Concrete nouns, real verbs, named mechanisms. Adjectives
are what you reach for when you don't know the specific  so find the specific instead. Every time you
write "innovador", "lder", "robusto", ask what concrete fact you're using it to avoid stating.
...
Register:    {t / usted / vosotros}    {formalcasual on a 15}
Market:      {es-PE / es-CO / en-US ...}    Currency: {S/ / USD / COP}
Verbs we use:    {action verbs that fit  "vende, activa, cierra, llega"}
Words we avoid:  {bannlist  "soluciones integrales, de clase mundial, nete hoy!"}
...
and what's the territory no competitor owns? Produce: the core promise (one sentence), 23 tagline
...
eyebrow, H1, subtitle, body, feature lines, CTA text + destination. For the **hero H1, always give 23
...
text, **error states** (specific and kind  "Ingresa un email vlido" not "Error"), **success states**
...
- Register: t/usted consistent on every page? (Mixing them is the classic tell.)
...
PAGE {n}: {name} ({/slug})
  SECTION {name}
    Eyebrow:  "..."
    H1:       A) "..."   angle   B) "..."   angle   C) "..."   angle      (hero: 23 options)
    Subtitle: "..."
    Body:     "..."
    Features: "..."  "..."  "..."
...
Weak:   "La mejor plataforma de subastas del pas"   (adjective, unprovable, says nothing specific)
Strong: "Vende tus activos ante compradores que compiten en vivo"  (mechanism + benefit, concrete)

**CTA  vague vs specific**
Weak:   "Enviar" / "Ms informacin"
Strong: "Solicitar consulta comercial"  #consulta   (verb + object, names the action and outcome)

**Error state  cold vs usable**
Weak:   "Campo invlido"
Strong: "Ingresa un telfono vlido, con cdigo de pas"   (says what's wrong AND how to fix it)

**Stat  fabricated vs honest**
Weak:   "Ms de 140,000 usuarios confan en nosotros"   (unsourced = a liability on a 'transparent' brand)
