---
name: apex-health
description: Read-only health sweep of the APEX Ultra runtime (M1) — probes every surface (loop.py/snapshot, Schwab token, council_loop board, signal plans, scanner/plan_generator via watchdog, sa_news_feed, MASTERS, HERMES/Slack) and returns a verdict table with evidence, using a failure-signature library so known incidents are recognized in seconds. Use whenever the user says "system health check", "health check", "sweep the system", "is apex alive", "is everything running", "apex status", "estado del sistema", "is the loop up", "why is the board stale", "why no alerts today", "snapshot looks frozen", "check the feeds", or asks ANY is-it-up / why-is-it-quiet / why-is-this-stale question about APEX Ultra, even casually mid-session. Also fire proactively before relaying any council verdict or trading-state answer when data freshness is uncertain. Diagnoses and prescribes only — never fixes (builder-handoff) and never deliberates (council-run).
---
